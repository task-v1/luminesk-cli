...
input selects the host-side Docker port; Paper listens on its default container
port. The memory input is used both in Java's argv and Docker's memory limit.
...
There is intentionally no `server.properties` template. Paper creates its
complete default file on first start. A two-line template would replace that
whole file, not merge two properties into an upstream default.
...
Top-level template output is `generated` by default. The ownership declaration
reserves `server.properties` for the server and operator. Because the recipe
does not package that path, Paper can create the complete file on first start
and later package updates leave it alone.
...
Template rendering always produces a complete file; it never merges individual
properties into a server-owned file. Prefer omitting editable server configs
from the package and declaring them `preserve`. If a recipe genuinely needs to
...
# Edit the complete server.properties generated by Paper.
nesk start --dir ./test-paper
```

Inspect the rendered EULA, complete server-generated configuration, ownership
